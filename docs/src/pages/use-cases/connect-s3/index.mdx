...
- This new IAM user should contain the [`AmazonS3FullAccess` policy](https://console.aws.amazon.com/iam/home?region=us-east-1#/policies/arn:aws:iam::aws:policy/AmazonS3FullAccess$serviceLevelSummary) attached to it _(as it will need both read and write access to S3)_.
-  An [S3 Bucket Policy](https://docs.aws.amazon.com/AmazonS3/latest/dev/walkthrough1.html) is required to be set on the Bucket to allow the IAM user to perform CRUD actions on the bucket and its objects.
  ```json
  {
    "Id": "PolicyId1234",
    "Version": "2012-10-17",
    "Statement": [
      {
        ...
        "Principal": {
          "AWS": [
            "arn:aws:iam::{aws-account-id}:user/{new-iam-user}"
          ]
        }
      }
    ]
...
2. Check out the `camel-kafka-connector-0.1.0-branch`:
   - `git checkout camel-kafka-connector-0.5.0`
3. To reduce the overall build scope of the project, you can comment out the undesired modules from the `<modules>` element of the `connectors/pom.xml` using `<!-- -->` notation.
4. From the root directory of the repository, build the project components:
   - `mvn clean package`
5. Go get a coffee and take a walk... as this build will take around 30 minutes on a normal developer workstation for a full build.
6. Copy the generated S3 artifacts to a staging area for the OpenShift build process:
  ```bash
  unzip -d connector-build ./connectors/camel-aws2-s3-kafka-connector/target/camel-aws2-s3-kafka-connector-0.5.0-package.zip
  ```
...
1. From the root directory of the repository, start a new OpenShift Build, using the generated build artifacts:
   ```shell
   oc start-build connect-cluster-101-connect --from-dir=./connector-build/ --follow
   ```
...
- The S3 Bucket parameter of the `camel.sink.path.bucketNameOrArn` configuration option _(named `my-s3-bucket` here)_

<InlineNotification kind="warning">
**TODO:** WORK IN PROGRESS - Refine parameters
- [ ] The configuration below does not allow for the previous parameterization of the S3 KeyName
  - This seems to be an issue with the aws2-s3 configuration options.
  - This should work, as per https://github.com/apache/camel-kafka-connector-examples/tree/master/aws2-s3/aws2-s3-sink-with-zip-aggregation#create-connector-instance
</InlineNotification>
...
spec:
  ...
  config:
    ...
    #camel.sink.url: aws2-s3://my-s3-bucket?keyName=${date:now:yyyyMMdd-HHmmssSSS}-${exchangeId}
    camel.sink.path.bucketNameOrArn: my-s3-bucket
    camel.sink.endpoint.keyName: ${date:now:yyyyMMdd-HHmmssSSS}-${exchangeId}
    camel.component.aws2-s3.keyName: ${date:now:yyyyMMdd-HHmmssSSS}-${exchangeId}
...
- Update the YAML and save it in a file named `kafka-sink-connector.yaml`
- Create this resource via `kubectl apply -f kafka-sink-connector.yaml`.
- You can then tail the output of the `connect-cluster-101` pods for updates on the connector status.
...
- [x] The configuration below does not use the S3ObjectConverter class, as recommended by the docs due to ClassCastExceptions. (Per https://github.com/apache/camel-kafka-connector/issues/338, the special class is not needed...)
- [ ] The configuration below does not set a Kafka Record key.
- [ ] Document `camel.component.aws2-s3.moveAfterRead`, along with `camel.component.aws2-s3.destinationBucket`.
</InlineNotification>
...
- The `strimzi.io/cluster` label must match the deployed Kafka Connect cluster you previously deployed _(or else Strimzi will not connect the `KafkaConnector` to your `KafkaConnect` cluster)_
- The `topics` parameters _(named `my-target-topic` here)_
- The S3 Bucket parameter of the `camel.source.path.bucketNameOrArn` configuration option _(named `my-s3-bucket` here)_
...
spec:
  ...
  config:
    ...
    camel.source.path.bucketNameOrArn: my-s3-bucket
...
- Update the YAML and save it in a file named `kafka-source-connector.yaml`.
- Create this resource via `kubectl apply -f kafka-source-connector.yaml`.
- You can then tail the output of the `connect-cluster-101` pods for updates on the connector status.

**NOTE:** If you require the connector to only read objects from a subdirectory of the S3 bucket root, you can set the `camel.component.aws-s3.configuration.prefix` configuration option with the value of the subdirectory name. For example, `camel.component.aws-s3.configuration.prefix: myfoldername` .
